...
from app.Component import Balloon
from constants import (
    BAND_FILTER,
    DECISION_CONTROL_CATEGORY,
    EEG_INPUT_DURATION,
    FILE_TYPES_MODEL,
...
from ui.uiDecisionControl import uiDecisionControl
from utils import band_filter, eeg_data_convert


class DecisionControl(uiDecisionControl):
    def __init__(self, master, window_height, window_width):
        super(DecisionControl, self).__init__(master, window_height, window_width)

        self.master = master
        self.category = np.array(DECISION_CONTROL_CATEGORY)
        ...
        self.sky = pygame.image.load(SKY_IMG)
        self.sky = pygame.transform.scale(
            self.sky, (self.pygame_frame_width, self.pygame_frame_height)
        )

        self.stimuli_go_img = pygame.image.load(STIMULI_GO_IMG)
        self.stimuli_go_img = pygame.transform.scale(
            self.stimuli_go_img, (self.pygame_frame_width, self.pygame_frame_height)
        )

        self.stimuli_stop_img = pygame.image.load(STIMULI_STOP_IMG)
        self.stimuli_stop_img = pygame.transform.scale(
            self.stimuli_stop_img, (self.pygame_frame_width, self.pygame_frame_height)
        )
        self.stimulate_img = {"go": self.stimuli_go_img, "stop": self.stimuli_stop_img}

        balloon_height = self.pygame_frame_height // 2
        balloon_size = (balloon_height, balloon_height)

        balloon_pos = (self.pygame_frame_width // 2, self.pygame_frame_height * 0.57)
    ...
    def model_initialize(self):
        try:
            self.model = SINCEEGNet(num_class=2)
    ...
    def draw_background(self):
        self.pygame_surface.blit(self.sky, [0, 0])
        # self.balloon.draw(self.pygame_surface)
    ...
    def stimulate(self):
        self.pygame_surface.blit(self.stimulate_img[self.gt_direction], [0, 0])
    ...
    def model_inference(self):
        ...
        self.update_stat()
        self.display_balloon_animation()
    ...
    def display_balloon_animation(self):
        if not self.balloon._done_moving:
            self.pygame_surface.blit(self.stimulate_img[self.gt_direction], [0, 0])
            self.balloon.move(direction=self.pred_direction)
            self.balloon.draw(self.pygame_surface)
            pygame.display.update()
            self.Pygame_Frame.after(
                FRAME_UPDATE_INTERVAL, self.display_balloon_animation
            )
        else:
            self.balloon._done_moving = False
            self.balloon_end_time = time.time() * 1000
            self.display_result()

    def display_result(self):
        if time.time() * 1000 - self.balloon_end_time < RESULT_DISPLAY_TIME:
            self.draw_background()
            if self.gt_direction == self.pred_direction:
                self.balloon.draw_correct(self.pygame_surface)
            else:
                self.balloon.draw_wrong(self.pygame_surface)
            pygame.display.update()
            ...
            self.trial_start_time = time.time() * 1000
            self.draw_background()
            self.balloon.reset(self.pygame_surface)
            self.trail_index += 1
            self.do_single_trail()
